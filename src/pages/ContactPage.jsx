...
export default function ContactPage() {
  ...
  return (
    <div style={{ backgroundColor: '#0E0E0E', minHeight: '100vh' }}>
      ...
      {/* Form + Connect */}
      <section style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 1.5rem 5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem' }}>

        {/* Form */}
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <div>
            <label style={labelStyle}>Name</label>
            <input type="text" placeholder="IDENTITY" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required style={{ ...inputStyle, caretColor: '#A7A5FF' }}
          ...
          <div>
            ...
            <input type="email" placeholder="PROTOCOL@DOMAIN.COM" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required style={{ ...inputStyle, caretColor: '#A7A5FF' }}
              onFocus={e => e.target.style.borderBottomColor = 'rgba(167,165,255,0.6)'}
              onBlur={e => e.target.style.borderBottomColor = 'rgba(255,255,255,0.15)'}
            />
          </div>
          <div>
            ...
            <textarea placeholder="SHARE WHAT IS ON YOUR MIND..." value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={5} required
              style={{ ...inputStyle, resize: 'none', caretColor: '#A7A5FF' }}
              onFocus={e => e.target.style.borderBottomColor = 'rgba(167,165,255,0.6)'}
              onBlur={e => e.target.style.borderBottomColor = 'rgba(255,255,255,0.15)'}
            />
          </div>
          {status === 'sent'  && <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#A7A5FF' }}>Message sent! We'll be in touch.</p>}
          {status === 'error' && <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#FF7439' }}>Something went wrong. Email us directly.</p>}
          <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
          <div>
            <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#FF7439', marginBottom: '1.25rem' }}>
              Connect With Us
            </p>
          ...
          <div style={{ border: '1px solid rgba(255,255,255,0.1)', padding: '1.5rem', borderLeft: '3px solid #A7A5FF' }}>
            ...
            <a href="mailto:devvit.work@gmail.com" style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 700, color: '#fff', fontSize: '1.25rem', textDecoration: 'none', transition: 'color 0.2s' }}
              onMouseEnter={e => e.target.style.color = '#A7A5FF'}
              onMouseLeave={e => e.target.style.color = '#fff'}
            >
              devvit.work@gmail.com
            </a>
          </div>
          ...
          <div>
            ...
            <div style={{ border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden' }}>
              <iframe
                title="SRM Location"
                src="https://maps.google.com/maps?q=SRM+Institute+of+Science+and+Technology&output=embed"
