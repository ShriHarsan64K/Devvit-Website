...
function useReveal(delay = 0) {
  ...
  useEffect(() => {
    ...
    if (delay) { setTimeout(() => observer.observe(el), delay) }
  ...
  return (
    <div style={{ backgroundColor: '#0E0E0E', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div ref={heroRef} className="reveal" style={{ position: 'relative', zIndex: 10, maxWidth: '80rem', margin: '0 auto', padding: '0 1.5rem', width: '100%', paddingTop: '4rem' }}>
          ...
          <p className="reveal reveal-delay-2" style={{ fontFamily: 'Inter, sans-serif', color: 'rgba(255,255,255,0.6)', fontSize: '1.125rem', marginTop: '2rem', marginBottom: '2.5rem', lineHeight: 1.7 }}>
      ...
      <section style={{ maxWidth: '80rem', margin: '0 auto', padding: '5rem 1.5rem' }}>
        <h2 ref={whatRef} className="reveal" style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 900, color: '#fff', fontSize: 'clamp(2.5rem, 5vw, 4rem)', marginBottom: '3rem', letterSpacing: '-0.02em' }}>
          WHAT WE DO
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
          ...
          <div style={{ position: 'relative', minHeight: '340px', overflow: 'hidden', cursor: 'pointer' }}
            onMouseEnter={e => { e.currentTarget.querySelector('div').style.opacity = '0.9'; e.currentTarget.querySelector('div').style.transform = 'scale(1.05)' }}
            onMouseLeave={e => { e.currentTarget.querySelector('div').style.opacity = '0.7'; e.currentTarget.querySelector('div').style.transform = 'scale(1)' }}
          >
            <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=800&q=80')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.7, transition: 'opacity 0.5s ease, transform 0.5s ease' }} />
          ...
          <div style={{ position: 'relative', minHeight: '340px', overflow: 'hidden', cursor: 'pointer' }}
            ...
            onMouseLeave={e => { e.currentTarget.querySelector('div').style.opacity = '0.7'; e.currentTarget.querySelector('div').style.transform = 'scale(1)' }}
          >
            <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('https://images.unsplash.com/photo-1531482615713-2afd69097998?w=800&q=80')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.7, transition: 'opacity 0.5s ease, transform 0.5s ease' }} />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.85) 40%, transparent)' }} />
            <div style={{ position: 'absolute', bottom: 0, left: 0, padding: '2rem' }}>
      ...
      <section style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 1.5rem 5rem' }}>
        <h2 ref={eventsRef} className="reveal" style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 900, color: '#fff', fontSize: 'clamp(2.5rem, 5vw, 4rem)', marginBottom: '0.5rem', letterSpacing: '-0.02em' }}>UPCOMING EVENTS</h2>
        <div style={{ width: '5rem', height: '3px', backgroundColor: '#A7A5FF', marginBottom: '3rem' }} />
        <a href="https://luma.com/yq94wzmt" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'flex', gap: '1.5rem', alignItems: 'center', border: '1px solid rgba(255,255,255,0.1)', padding: '1.5rem', transition: 'all 0.3s ease' }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
          <div ref={whyRef} className="reveal">
            ...
            <ul style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', listStyle: 'none' }}>
              {whyDevvit.map((item, i) => (
                <li key={item.num} className={`reveal reveal-delay-${i + 1}`} style={{ display: 'flex', gap: '1.5rem' }}>
          ...
          <div style={{ overflow: 'hidden' }}>
            <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=800&q=80" alt="Why Devvit" loading="lazy" style={{ width: '100%', height: '450px', objectFit: 'cover', filter: 'grayscale(100%)', transition: 'transform 0.6s ease' }}
      ...
      {/* ── JOIN CTA ─────────────────────────────────────── */}
      <section ref={joinRef} className="reveal" style={{ padding: '6rem 1.5rem', textAlign: 'center' }}>
